...
import { Heart, X, Sparkles, Calendar, MapPin, Users, Zap } from "lucide-react"
...
export function MatchCarousel() {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      await SheetsService.saveMatch({
        participants: currentMatch.members.map((m) => m.name),
        activity: currentMatch.suggestedActivity,
      ...
      await SlackService.sendMatchNotification(
        currentMatch.members.map((m) => m.name),
      ...
      // Show celebration animation instead of simple success
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-jungle-background">
        <motion.div initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center">
          ...
          <h2 className="text-4xl font-bold text-jungle-accent mb-4">Match Made!</h2>
          <p className="text-xl text-jungle-textLight/70 mb-6">You&apos;ve joined &quot;{matchedActivity}&quot;</p>
          <motion.div
            animate={{ scale: [1, 1.1, 1] }}
            transition={{ duration: 1, repeat: Number.POSITIVE_INFINITY }}
            className="text-jungle-accent"
          >
            <Sparkles className="w-12 h-12 mx-auto" />
          </motion.div>
          <p className="text-sm text-jungle-textLight/50 mt-4">Slack notification sent to all participants</p>
        </motion.div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-jungle-background">
        <Card className="glass-effect border-white/20 text-center p-8">
          ...
          <h2 className="text-2xl font-bold text-jungle-accent mb-2">No More Matches</h2>
          <p className="text-jungle-textLight/70">Check back later for new connection opportunities!</p>
          <Button
            className="mt-4 bg-jungle-accent text-jungle-textDark hover:bg-jungle-accent/90"
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-jungle-background">
      ...
      <MatchCelebration
        ...
        matchedUser={{ name: currentMatch.members[0]?.name || "" }}
      />
          ...
          >
            <Card className="glass-effect border-white/20 overflow-hidden">
              <CardHeader className="text-center pb-4">
                ...
                <CardTitle className="text-2xl text-jungle-accent mb-2">{currentMatch.suggestedActivity}</CardTitle>
                <div className="flex items-center justify-center text-sm text-jungle-textLight/70 space-x-4">
              ...
              <CardContent className="space-y-6">
                ...
                <div>
                  <div className="flex items-center mb-3">
                    <Users className="w-4 h-4 mr-2 text-jungle-accent" />
                    <span className="text-sm font-medium text-jungle-textLight">
                      Who's joining ({currentMatch.members.length})
                    </span>
                  </div>
                    ...
                    {currentMatch.members.map((member, index) => (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <p className="font-medium text-jungle-textLight">{member.name}</p>
                          <p className="text-sm text-jungle-textLight/70">{member.role}</p>
                        </div>
                ...
                <div>
                  <div className="flex items-center mb-3">
                    <Sparkles className="w-4 h-4 mr-2 text-jungle-accent" />
                    <span className="text-sm font-medium text-jungle-textLight">Shared interests</span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {currentMatch.sharedInterests.map((interest) => (
                      <Badge
                        key={interest}
                        variant="secondary"
                        className="bg-jungle-accent/20 text-jungle-accent border-jungle-accent/30"
                      >
                ...
                <div className="flex space-x-3 pt-4">
                  ...
                  <motion.div className="flex-1" whileTap={{ scale: 0.95 }}>
                    <Button
                      className="w-full h-14 bg-jungle-accent text-jungle-textDark hover:bg-jungle-accent/90 glow-effect"
                      onClick={handleJoin}
                      disabled={isLoading}
                    >
                      {isLoading ? (
            ...
            {matches.map((_, index) => (
              <div
                ...
                className={`w-2 h-2 rounded-full transition-colors ${
                  index === currentMatchIndex ? "bg-jungle-accent" : "bg-white/20"
                }`}
              />
            ))}
          </div>
